...
export = (app: Application) => {
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      bloqueioArray.forEach((bloqueio: BloqueioAgendamentoModel) => {
        //validações unitárias das props de bloqueio
      ...
      if (erroBool)
        return next(createError(HTTP_ERRORS.BAD_REQUEST, erroMsgUnificado));

      //faz a tipagem do array e converte a data para o tipo necessário
      const bloqueiosMapeados: BloqueioAgendamentoModel[] = bloqueioArray.map(
  ...
  app.put(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      let bloqueio: BloqueioAgendamentoModel =
        await BloqueioAgendamento.getBloqueioAgendamentoById(bloqueioId);
      ...
      if (usuario.tipo_usuario != TipoUsuario.superAdmin)
        return next(
          createError(
            ...
            'Você não tem permissão para atualizar esse bloqueio!'
          )
        );
