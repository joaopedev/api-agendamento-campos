...
export = (app: Application) => {
      ...
      if (!agendamento.usuario_id || !agendamento.data_hora) {
        ...
        return next(
          createError(
            ...
            `É obrigatório informar a propriedade ${erro}`
          )
        );
      }
  ...
  app.put(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      let agendamento: SchedulingModel = await Scheduling.getScheduleById(
        agendamentoId
      );
  ...
  app.delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      let agendamentoDelete: SchedulingModel = await Scheduling.getScheduleById(
        agendamentoId
      );
